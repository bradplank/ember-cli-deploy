...
module.exports = Task.extend({
  run: function(options) {
    ...
    if(indexFiles) {
      ui.writeLine(chalk.blue('\nUploading '+indexFiles.length+' files from `' + config.get('outputPath') + '/`...\n'));
      return deploy.uploadFiles(indexFiles);
    } else {
      return readFile(config.get('outputPath') +'/index.html')
        .then(function(fileContent) {
          ui.writeLine(chalk.blue('\nUploading `' + config.get('outputPath') + '/index.html`...\n'));
